...
public sealed partial class ParrotAccentSystem : EntitySystem
{
    ...
    private void OnAccentGet(Entity<ParrotAccentComponent> entity, ref AccentGetEvent args) 
        => args.Message = args.TTSMessage = Accentuate(entity, args.Message);

    // Darkrell: I tested it. It seems to work fine, but I am already exhausted, so I did not check very carefully. If anything, we can redo it later.
