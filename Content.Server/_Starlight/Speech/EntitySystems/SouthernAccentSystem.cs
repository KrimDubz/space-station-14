...
public sealed partial class SouthernAccentSystem : EntitySystem
{
    ...
    {
        (args.Message, args.TTSMessage) = _replacement.ApplyReplacements((args.Message, args.TTSMessage), "southern");

        args.Message = RegexIng().Replace(args.Message, m => PreserveCase(m.Value, "in'"));
        args.Message = RegexAnd().Replace(args.Message, m => PreserveCase(m.Value, "an'"));
        args.Message = RegexDve().Replace(args.Message, m => PreserveCase(m.Value, "da"));
    }
